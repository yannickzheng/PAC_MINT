...
class Room:
    ...
    def __init__(self, player_capacity, room_name, code=None):
        ...
        self.players = (
            {}
        )  #  Exemple : {"pacman": Player(...), "fantome_1": Player(...)}
    ...
    def join(self, player_id, role=None):
        ...
        if role and self.is_role_taken(role):
            ...
            print(
                f"[DEBUG][Room] join refusé: rôle {role} déjà pris. Rôles pris: {taken_roles}"
            )
            return {
                "status": "error",
                "reason": "role_taken",
                "message": f"Le rôle '{role}' est déjà pris dans cette salle.",
                "taken_roles": taken_roles,
            }  # Rôle déjà pris : refuse l’entrée
    ...
    def add_chat_message(self, player_id, message):
        ...
        player_name = (
            self.players[player_id].role if player_id in self.players else "Joueur"
        )
