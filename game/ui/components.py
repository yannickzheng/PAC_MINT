    ...
    if hover:
        for i in range(10):
            ...
            glow_surface = pygame.Surface(
                (width + glow_radius * 2, height + glow_radius * 2), pygame.SRCALPHA
            )
            pygame.draw.rect(
                glow_surface,
                (*glow_color, 50 - i * 5),
                (0, 0, width + glow_radius * 2, height + glow_radius * 2),
                border_radius=10,
            )
...
def game_over(score, screen, font):
    ...
    instruction_text = instruction_font.render(
        "Appuyez sur une touche pour continuer...", True, (200, 200, 200)
    )
