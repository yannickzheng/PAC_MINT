...
class Player:
    ...
    def draw(self, screen, controlled=None):
        """Affiche le joueur à l'écran"""
        if self.is_pacman:
            screen.blit(self.get_img_pacman(controlled), (int(self.x), int(self.y)))
    ...
    def get_img_pacman(self, controlled_player):
        ...
        if self != controlled_player:
            ...
            if getattr(self, 'invincible', False) and (getattr(self, 'invincibility_timer', 0) // 10) % 2 == 0:
            ...
            if getattr(self, 'super_power_active', False):
                if direction == 'left': return self.image_super_left
                elif direction == 'right': return self.image_super_right
                elif direction == 'up': return self.image_super_up
                elif direction == 'down': return self.image_super_down
    ...
    def handle_collisions_with_players(self, players):
        """Gère les collisions entre PacMan et les fantômes"""
        if not self.is_pacman or getattr(self, 'invincible', False):
            return
    ...
    def is_position_free(self, x, y, players):
        ...
        for player in players.values():
            if player == self:
                continue
            dx = (player.x + player.size // 2) - (x + self.size // 2)
            dy = (player.y + player.size // 2) - (y + self.size // 2)
            distance_squared = dx * dx + dy * dy
    ...
    def find_path(self, start, goal, map_data):
        """Algorithme A* basique pour trouver un chemin sur la MAP_DATA"""
        open_set = []
    ...
    def ghost_ai_move(self, pacman):
        """IA pour les fantômes avec algorithme A*"""
        if getattr(self, 'is_eaten', False):
        ...
        if getattr(pacman, 'super_power_active', False) and getattr(self, 'pathfinding_timer', 0) > 0:
            self.pathfinding_timer = 0

        self.pathfinding_timer = getattr(self, 'pathfinding_timer', 0) - 1

        if self.pathfinding_timer <= 0 or not getattr(self, 'current_path', []):
            ...
            self.pathfinding_timer = 5  # Recalcul toutes les 5 frames
    ...
    def move(self, players, controlled=False):
        ...
        if getattr(self, 'super_power_active', False):
            self.super_power_timer = getattr(self, 'super_power_timer', 0) - 1
            if self.super_power_timer <= 0:
                self.super_power_active = False
                self.speed = CELL_SIZE // 6

        if self.is_phantom and getattr(self, 'is_eaten', False):
            self.update_eaten_state()
            self.update()
            return

        if self.is_pacman or (self.is_phantom and controlled):
            # Contrôle clavier pour Pacman et pour le fantôme contrôlé par le joueur
    ...
    def update_eaten_state(self):
        ...
        if not self.is_phantom or not getattr(self, 'is_eaten', False) or not getattr(self, 'respawn_target', None):
            return
...
class PacMan(Player):
    ...
    def handle_collisions_with_ghosts(self, players):
        ...
        for player in players.values():
            if player != self and isinstance(player, Ghost):
                ghost_center = (player.x + player.size // 2, player.y + player.size // 2)
                dx = pacman_center[0] - ghost_center[0]
                dy = pacman_center[1] - ghost_center[1]
                distance_squared = dx * dx + dy * dy
